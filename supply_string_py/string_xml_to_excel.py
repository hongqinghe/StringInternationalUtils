...
def getSheetWs(wb, sheet_title):
    ...
    ws.append(
        {1: SC._excel_title_value_key, 2: SC._excel_title_value_value, 3: SC._excel_title_value_en,
         4: SC._excel_title_value_thai})
...
def xmlParse(zhPath, enPath, thaiPath, ws):
    ...
    for zh_elem in zh_tree.iter(tag='string'):
        ...
        if en_value == '':
            ...
            cell = ws['C' + str(currentRow)]
            enEmptyColor = 'dda0dd'
            cell.fill = PatternFill(fill_type='lightGray', bgColor=enEmptyColor,
                                    fgColor=enEmptyColor)
        # if thai_value == '' or  re.findall('%s',zh_value)==1:
        if not zh_value == None:
            if (zh_value.count('%s') > 0 and thai_value.count('%s') <= 0) or (
                    zh_value.count('%d') > 0 and thai_value.count('%d') <= 0) or (
                    zh_value.count('%1$s') > 0 and thai_value.count('%1$s') <= 0) or (
                    thai_value == '') or (zh_value.count('\n') > 0 and thai_value.count('\n') <= 0):
                currentRow = ws._current_row
                cell = ws['D' + str(currentRow)]
...
def setCellProperty(worksheet):
    for i in range(1, worksheet.max_row + 1):
        ...
        for col in range(ord('A'), ord('E')):
            ...
            borderStype = 'thick'
            borderColor = '556b2f'
            worksheet[chr(col) + str(i)].border = Border(
                left=Side(border_style=borderStype, color=borderColor),
